...
#define  GET_LOG_PATH_MACRO \
                FILE *dout1; \
                char log_path_getcd[100];\
                char logbuffile[100];\
                getcwd(log_path_getcd,100);\
                log_path_getcd[strlen(log_path_getcd)-LOG_DIR_TRUNCATE]=0;\
                sprintf(logbuffile, "%s/log/boa.log" , log_path_getcd );\
                dout1 = fopen(logbuffile, "a"); 
...
#define DEBUGINFO2(a,b) \
        { \
		GET_LOG_PATH_MACRO  \
                fprintf(dout1,a,b); \
                fclose(dout1);}

#define DEBUGINFO3(a,b,c) \
        { \
		GET_LOG_PATH_MACRO  \
                fprintf(dout1,a,b,c); \
                fclose(dout1);}
